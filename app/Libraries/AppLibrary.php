    ...
    public static function currencyAmountFormat($amount): string
{
    ...
    $position = env('CURRENCY_POSITION', 'RIGHT');

    $formattedAmount = number_format($amount, $decimalPoint, '.', '');
    $ltr = "\u{200E}"; // ← هذه العلامة تمنع انقلاب النص

    if ($position === CurrencyPosition::LEFT) {
        return $ltr . $symbol . ' ' . $formattedAmount;
    }

    return $ltr . $formattedAmount . ' ' . $symbol;
}
