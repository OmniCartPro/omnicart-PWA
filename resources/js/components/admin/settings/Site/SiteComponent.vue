...
<div class="form-col-12 sm:form-col-6">
    <label for="site_theme_color" class="db-field-title required">
         {{ $t("label.primary_color") }}
    </label>
...
/>
    ...
    <small class="db-field-alert" v-if="errors.site_theme_color">
        {{ errors.site_theme_color[0] }}
    </small>
</div>
...
export default {
    ...
    data() {
        return {
            ...
            form: {
             ...
             hexValue: '#ffffff', // هيتم حسابه عند mount
        errors: {},
            enums: {
    ...
    mounted() {
        this.load();
        if (this.form.site_theme_color) {
        this.hexValue = this.rgbToHex(this.form.site_theme_color);
    } else {
        // لو مفيش قيمة محفوظة، خليه يختار الافتراضي من color picker
        this.form.site_theme_color = this.hexToRgb(this.hexValue);
    }
    },
    methods: {
        floatNumber(e) {
        ...
        list: function () {
            ...
            this.$store.dispatch('site/lists').then(res => {
                this.form = {
                    ...
                    site_theme_color: res.data.data.site_theme_color,
                }
                        if (this.form.site_theme_color) {
                            this.hexValue = this.rgbToHex(this.form.site_theme_color);
                        } else {
                            this.form.site_theme_color = "0 90 98";
                            this.hexValue = this.rgbToHex(this.form.site_theme_color);
                        }
                this.loading.isActive = false;
            }).catch((err) => {
