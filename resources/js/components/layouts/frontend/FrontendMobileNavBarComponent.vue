<template>
    <nav class="lg:hidden w-full flex items-center justify-between px-5 py-3 fixed bottom-0 left-0 z-10 shadow-widget bg-white">
        ...
        <button @click="showTarget('cart-canvas', 'canvas-active')" type="button" class="relative isolate -mt-11">
            ...
            <span v-if="carts.length > 0" class="absolute top-5 ltr:right-1.5 rtl:left-1.5 text-[10px] font-medium h-4 px-1 leading-[14px] text-center rounded-full border border-primary bg-[#000000] text-[#ffffff]">
                {{ carts.length }}
            </span>
        </button>
